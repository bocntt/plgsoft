...
class coreorders_db {
	...
	function get_total_coreorders($array_keywords=array()) {
		...
		$sql = "SELECT COUNT(autoid) AS cnt FROM " .$table_name.
				" WHERE 1=1";
	...
	function get_all_coreorders($array_keywords=array(), $limit=0, $offset=0) {
		...
		foreach ($list_results as $item_obj) {
			...
			$results[$index]['autoid'] = $item_obj->autoid;
	...
	function get_plgsoft_coreorders_by_id($autoid) {
		...
		if (strlen($table_name) == 0) {
			...
			$sql = "SELECT * " .
				" FROM " . $table_name . " WHERE 1=1 AND autoid='" . $autoid . "'";
			$item_obj = $wpdb->get_row($sql);
			if(isset($item_obj)) {
				$result_obj['autoid'] = $item_obj->autoid;
