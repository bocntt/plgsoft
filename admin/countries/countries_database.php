...
class countries_database {
	...
	function get_total_plgsoft_countries($array_keywords=array()) {
		...
		if (strlen($table_name) == 0) {
			...
			$sql = "SELECT COUNT(country_key) FROM " . $table_name;
			$sql .= " WHERE 1=1";
			if (strlen($keyword) > 0) {
	...
	function get_list_plgsoft_countries($array_keywords=array(), $limit=0, $offset=0) {
		...
		if (strlen($table_name) == 0) {
			...
			$sql = "SELECT country_key, country_name, is_active, order_listing, seo_title, seo_description " .
				" FROM " . $table_name;
			$sql .= " WHERE 1=1";
			if (strlen($keyword) > 0) {
				$sql .= " AND (LOWER(country_name) LIKE '%".$keyword."%')";
	...
	function check_exist_country_name($country_name, $country_key = '') {
		global $wpdb;
		...
		if (strlen($table_name) == 0) {
			...
			$sql = "SELECT COUNT(country_key) " .
				" FROM " . $table_name .
				" WHERE LOWER(country_name)='".esc_sql($country_name)."' AND country_key <> '".$country_key."'";
			$total_countries = $wpdb->get_var($sql);
	...
	function check_exist_country_key($country_key) {
		global $wpdb;
		...
		if (strlen($table_name) == 0) {
			...
			$sql = "SELECT COUNT(country_key) " .
				" FROM " . $table_name .
				" WHERE LOWER(country_key)='".esc_sql($country_key)."";
			$total_countries = $wpdb->get_var($sql);
			if ($total_countries == 0)
				return false;
			else
	...
	function delete_plgsoft_countries($country_key) {
		global $wpdb;
		$table_name = $wpdb->prefix . $this->table_name;
		if ((strlen($country_key) > 0) && (strlen($table_name) > 0)) {
			global $wpdb;
			$sql = "DELETE FROM ".$table_name." WHERE country_key='" .esc_sql($country_key). "'";
			$wpdb->query($sql);
		}
	}

	function active_plgsoft_countries($country_key) {
		global $wpdb;
		$table_name = $wpdb->prefix . $this->table_name;
		if (strlen($table_name) > 0) {
			$sql = "UPDATE ".$table_name." SET is_active='1' WHERE country_key='%s'";
			$wpdb->query($wpdb->prepare($sql, $country_key));
		}
	}

	function deactive_plgsoft_countries($country_key) {
		global $wpdb;
		$table_name = $wpdb->prefix . $this->table_name;
		if (strlen($table_name) > 0) {
			$sql = "UPDATE ".$table_name." SET is_active='0' WHERE country_key='%s'";
			$wpdb->query($wpdb->prepare($sql, $country_key));
		}
	}
}
